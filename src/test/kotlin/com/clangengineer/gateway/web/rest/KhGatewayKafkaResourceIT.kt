...
import com.clangengineer.gateway.config.EmbeddedKafka
import com.clangengineer.gateway.config.KafkaSseConsumer
import com.clangengineer.gateway.config.KafkaSseProducer
...
@WithMockUser
@EmbeddedKafka
class KhGatewayKafkaResourceIT {
    ...
    fun producesMessages() {
        client.post().uri("/api/kh-gateway-kafka/publish?message=value-produce")
            .exchange()
            .expectStatus()
            .isNoContent
    ...
    fun consumesMessages() {
        ...
        val value = client
            .get()
            .uri("/api/kh-gateway-kafka/consume")
